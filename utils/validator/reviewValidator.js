...
exports.createReviewValidator = [
    ...
    check('product')
        ...
        .custom((val, { req }) => {
            ...
            return Review.findOne({ user: req.user._id, product: req.body.product }).then((res)=> {
                if (res) {
...
exports.updateReviewValidator = [
    check('id')
        ...
        .custom((val, { req }) => {
            // Check if logged user create review before
            return Review.findById(val).then((res)=> {
                if (!res) {
                    // console.log(`review` + res)
                    return Promise.reject(new Error(`There is no review with id ${val}`));
                }
                // console.log((res.user === req.user._id)) // => false
                // console.log((res.user.toString() === req.user._id.toString())) // => true
